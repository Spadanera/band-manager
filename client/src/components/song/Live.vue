<template>
  ...
  >
    <v-card class="max-height">
      <v-toolbar dense dark color="primary" v-bind:class="{ 'padding-right': $vuetify.breakpoint.mdAndUp }">
      ...
      <v-toolbar v-bind:class="{ 'padding-right': $vuetify.breakpoint.mdAndUp }" dense>
        <v-toolbar-title>
          {{ songToShow.title }}
        </v-toolbar-title>
        <v-spacer></v-spacer>
      ...
      <v-navigation-drawer temporary fixed id="live-song-setlist" v-model="songNavigation" right>
        <v-subheader>
      ...
      <div  v-bind:class="{ 'padding-right': $vuetify.breakpoint.mdAndUp }" id="lyrics-container">
        <v-card-text>
...
export default {
  ...
  computed: {
    ...
    songToShow() {
      return this.event.setlist.length ? this.event.setlist[this.selectedSong] : {};
    },
    songList() {
  ...
  watch: {
      ...
      selectedSong() {
          document.querySelector("#live-song-setlist .v-list-item--active").scrollIntoView();
          document.querySelector("#lyrics-container .v-card__text").scrollIntoView();
      }
  }
};
</script>
